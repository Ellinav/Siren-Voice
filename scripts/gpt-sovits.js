        ...
        <datalist id="siren-gsv-ref-list"></datalist>
            ...
            <div style="background: rgba(15, 23, 42, 0.4); border: 1px solid #334155; border-radius: 6px; padding: 15px;">
                ...
                <div class="siren-ext-setting-row siren-ext-flex-between" style="margin-bottom: 10px;">
                    <div class="siren-ext-setting-label">API Base URL</div>
                    <div style="display: flex; gap: 5px; flex: 1; max-width: 60%;">
                        <input type="text" id="siren-gsv-api" class="siren-ext-input" style="flex: 1;" placeholder="http://127.0.0.1:9880" value="http://127.0.0.1:9880">
                        <button id="siren-gsv-check-btn" class="siren-ext-btn siren-ext-btn-secondary" title="测试连接">
                ...
                <div class="siren-ext-setting-row siren-ext-flex-between">
                    <div class="siren-ext-setting-label">
                        ...
                        <small style="display:block; color: #64748b;">上传参考音频到后端custom_refs文件夹</small>
                    </div>
                    <div>
                ...
                <div style="display: flex; gap: 15px; align-items: flex-end; margin-bottom: 15px;">
                    ...
                    <div style="flex: 1;">
                        <div style="display:flex; justify-content:space-between; font-size: 0.85em; color: #94a3b8; margin-bottom: 4px;">
                            <span>句间停顿间隔</span><span id="siren-gsv-interval-val" style="color: #fbbf24;">0.3s</span>
                        </div>
                    ...
                    <div style="flex: 1; padding-bottom: 8px;">
                        <label style="display: flex; align-items: center; gap: 5px; cursor: pointer;">
                            ...
                            <span style="font-size: 0.85em; color: #cbd5e1;">启用并行推理</span>
                        </label>
                    </div>
                </div>
...
export function bindGptSovitsEvents() {
            ...
            if (res.ok) {
                ...
                if (window.toastr)
                    window.toastr.success(
                        `GSV 连接成功！版本: ${JSON.stringify(data)}`,
                    );
                fetchCustomRefs();
            } else {
    ...
    $("#siren-gsv-refresh-models").on("click", async function () {
        ...
        try {
            ...
            for (const v of versions) {
                try {
                    const res = await fetch(
                        `${apiBase}/classic_model_list/${v}`,
                    );
                    if (!res.ok) continue; // 如果后端不支持该版本，直接跳过，不报错

                    const data = await res.json();
                    console.log(
                        `[Siren GSV] 版本 [${v}] 获取到的原始数据:`,
                        data,
                    );
            ...
            $(".siren-gsv-char-gpt, .siren-gsv-emo-gpt").each(function () {
                ...
                if (
                    currentVal &&
                    allGptOptions.find((o) => o.value === currentVal)
                ) {
                    $(this).val(currentVal);
                }
            });
            ...
            $(".siren-gsv-char-sovits").each(function () {
                ...
                $(this)
                    .empty()
                    .append('<option value="">选择SoVITS模型</option>');
    ...
    $("#siren-gsv-char-list").on("click", ".siren-gsv-adv-btn", function () {
        ...
        $("#siren-gsv-mod-rep")
            ...
            .trigger("input");
        $("#siren-gsv-mod-seed").val(
            activeCharRowForAdv.attr("data-seed") || -1,
        );
    ...
    $("#siren-gsv-mod-save").on("click", function () {
        if (activeCharRowForAdv) {
            activeCharRowForAdv.attr(
                "data-speed",
                $("#siren-gsv-mod-speed").val(),
            );
            activeCharRowForAdv.attr(
                "data-temp",
                $("#siren-gsv-mod-temp").val(),
            );
            activeCharRowForAdv.attr(
                "data-topp",
                $("#siren-gsv-mod-topp").val(),
            );
            activeCharRowForAdv.attr(
                "data-topk",
                $("#siren-gsv-mod-topk").val(),
            );
    ...
    $("#siren-gsv-char-list, #siren-gsv-emo-list").on(
        ...
        function () {
            const isCharRow =
                $(this).closest("#siren-gsv-char-list").length > 0;
            $(this)
    ...
    // 委托情绪控制模式切换事件
    $("#siren-gsv-emo-list").on(
        "change",
        ".siren-gsv-emo-mode-sel",
        function () {
            const mode = $(this).val();
    ...
    $("#siren-gsv-save-btn").on("click", async function () {
        ...
        settings.tts.gptsovits.parallel_infer = $(
            "#siren-gsv-global-parallel",
        ).is(":checked");
        ...
        settings.tts.gptsovits.top_k = parseInt(
            $("#siren-gsv-global-topk").val(),
        );
        settings.tts.gptsovits.repetition_penalty = parseFloat(
            $("#siren-gsv-global-rep").val(),
        );
        settings.tts.gptsovits.seed = parseInt(
            $("#siren-gsv-global-seed").val(),
        );
        ...
        $("#siren-gsv-char-list .siren-gsv-row-item").each(function () {
            ...
            if (charName) {
                charList.push({
                    ...
                    // 默认参考音频配置
                    refPath: $(this)
                        .find(".siren-gsv-char-ref-path")
                        .val()
                        .trim(),
                    promptLang: $(this)
                        .find(".siren-gsv-char-prompt-lang")
                        .val(),
                    refText: $(this)
                        .find(".siren-gsv-char-ref-text")
                        .val()
                        .trim(),
                    // 高级参数 (从绑定的 data 属性读取)
        ...
        $("#siren-gsv-emo-list .siren-gsv-row-item").each(function () {
            ...
            if (charName && emotion) {
                ...
                if (mode === "audio") {
                    config.refPath = $(this)
                        .find(".siren-gsv-emo-ref-path")
                        .val()
                        .trim();
                    config.refText = $(this)
                        .find(".siren-gsv-emo-ref-text")
                        .val()
                        .trim();
                } else if (mode === "model") {
    ...
    $("#siren-gsv-test-generate").on("click", async function () {
        ...
        if (!text || !charName) {
            if (window.toastr)
                window.toastr.warning("请选择测试角色并输入台词！");
            return;
        }
...
function updateTestDropdowns() {
    ...
    if (chars.length > 0) {
        chars.forEach((c) => {
            $charSel.append(
                `<option value="${c.charName}">${c.charName}</option>`,
            );
        });
    ...
    const html = `
        <div class="siren-gsv-row-item siren-ext-setting-row" 
            ...
            <div style="display: flex; gap: 8px; width: 100%;">
                ...
                <select class="siren-ext-select siren-gsv-char-prompt-lang" style="${rowInputStyle} flex: 1;" title="参考音频语种">
    ...
    const html = `
        <div class="siren-gsv-row-item siren-ext-setting-row" style="display: flex; flex-direction: column; gap: 8px; padding: 12px; background: rgba(0,0,0,0.25); border-radius: 8px; border: 1px solid rgba(255,255,255,0.03);">
            <div style="display: flex; gap: 8px;">
                <select class="siren-ext-select siren-gsv-emo-char" data-current-val="${data.charName || ""}" style="${rowInputStyle} flex: 1;">
                    <option value="">选择角色</option>
                </select>
                <input type="text" class="siren-ext-input siren-gsv-emo-name" placeholder="情绪名" value="${data.emotion || ""}" style="${rowInputStyle} flex: 1;">
                <select class="siren-ext-select siren-gsv-emo-mode-sel" style="${rowInputStyle} flex: 1;">
            ...
            <div class="siren-gsv-emo-audio-group" style="display: ${mode === "audio" ? "flex" : "none"}; gap: 8px; padding-left: 12px; border-left: 2px solid #38bdf8;">
                <input type="text" list="siren-gsv-ref-list" class="siren-ext-input siren-gsv-emo-ref-path" placeholder="选择情绪参考音频..." value="${mode === "audio" ? data.refPath || "" : ""}" style="${rowInputStyle} flex: 1;">
                <input type="text" class="siren-ext-input siren-gsv-emo-ref-text" placeholder="情绪参考音频文本内容" value="${mode === "audio" ? data.refText || "" : ""}" style="${rowInputStyle} flex: 2;">
            </div>
            ...
            <div class="siren-gsv-emo-model-group" style="display: ${mode === "model" ? "flex" : "none"}; flex-direction: column; padding-left: 12px; border-left: 2px solid #a855f7; width: 100%;">
                ...
                <div style="display: flex; gap: 8px; margin-bottom: 8px;">
                    <select class="siren-ext-select siren-gsv-emo-gpt" style="${rowInputStyle} flex: 1.5;">
                        <option value="${data.gptModel || ""}">${formatModelNameForDisplay(data.gptModel) || "选择情绪GPT模型..."}</option>
                    </select>
                    <select class="siren-ext-select siren-gsv-emo-model-prompt-lang" style="${rowInputStyle} flex: 1;" title="独立参考音频语种">
                        ${buildLangOpts(data.promptLang)}
                    </select>
                    <input type="text" list="siren-gsv-ref-list" class="siren-ext-input siren-gsv-emo-model-ref-path" placeholder="独立参考音频(不填用角色默认)" value="${mode === "model" ? data.refPath || "" : ""}" style="${rowInputStyle} flex: 1.5;">
                </div>
