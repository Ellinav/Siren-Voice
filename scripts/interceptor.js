// 记得在顶部导入新函数
import { stripParentheticalAsides, stripWrappingPunctuation } from "./utils.js";
        ...
        if (/<(speak|phone|inner)[^>]*>/i.test(msg.mes)) {
          ...
          chat[i].mes = chat[i].mes.replace(
            ...
            (match, tagName, innerText) => {
              ...
              if (tagLower === "speak") {
                replacement = settings.tts.clean_speak_tags_replacement || "“”";
              } else if (tagLower === "phone") {
                replacement = settings.tts.clean_phone_tags_replacement || "“”";
              } else if (tagLower === "inner") {
                replacement = settings.tts.clean_inner_tags_replacement || "**";
              }

              // 拆分前缀和后缀 (支持长度为2的对称符号，或单个符号)
              let prefix =
                replacement.length === 2 ? replacement[0] : replacement;
              let suffix =
                replacement.length === 2 ? replacement[1] : replacement;

              // 核心清洗器 1：剔除中英文语气词及方括号
              let cleanText = stripParentheticalAsides(innerText);

              // 核心清洗器 2：剔除首尾的冗余引号和星号
