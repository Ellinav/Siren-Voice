    ...
    return `
        ...
        <div>
            <div style="margin-bottom: 20px;">
                <div style="display: flex; gap: 10px; margin-bottom: 10px; align-items: center;">
                    ...
                    <button id="siren-idx-select-files-btn" class="siren-ext-btn siren-ext-btn-secondary" title="选择需要上传的音频文件">
                        <i class="fa-solid fa-cloud-arrow-up" style="color:#38bdf8;"></i> 上传音频
                    </button>
            ...
            <div style="background: rgba(0,0,0,0.2); padding: 10px; border-radius: 6px; margin-bottom: 20px;">
                ...
                <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 12px;">
                    <label style="display: flex; align-items: center; gap: 5px; font-size: 0.85em; color: #cbd5e1; cursor: pointer;">
                        <input type="checkbox" id="siren-idx-param-dosample" checked style="accent-color: #10b981;"> 启用采样 (do_sample)
                    </label>
                    <label style="display: flex; align-items: center; gap: 5px; font-size: 0.85em; color: #cbd5e1; cursor: pointer;" title="作用于自然语言情绪控制">
                        <input type="checkbox" id="siren-idx-param-emorandom" style="accent-color: #a855f7;"> 情绪随机 (emo_random)
                    </label>
                </div>
            ...
            <button id="siren-idx-global-save" class="siren-ext-btn siren-ext-btn-primary" style="width: 100%; padding: 12px 0; justify-content: center; font-size: 1.05em; margin-top: 15px; background: linear-gradient(135deg, #0ea5e9, #10b981); border: none;">
                <i class="fa-solid fa-layer-group"></i> 同步保存全部配置 (全局参数 & 角色音色)
            </button>
...
export function loadCharacterTtsList() {
    ...
    if (context && context.characterId !== undefined) {
        ...
        if (charConfig && charConfig.voices) {
            Object.entries(charConfig.voices).forEach(
                ([charName, voicePath]) => {
...
export async function loadIndexTtsData() {
    ...
    // 基础与高级参数回显
    $("#siren-indextts-api").val(
        ttsSettings.api_base || "http://127.0.0.1:7880",
    );
    $("#siren-idx-param-dosample").prop(
        "checked",
        ttsSettings.do_sample ?? true,
    );
    ...
    if (Array.isArray(ttsSettings.emotion_presets)) {
        ttsSettings.emotion_presets.forEach((emo) => {
            ...
            if (emo.method === "vector") {
                ...
                $row.find(".siren-idx-vec-data").val(
                    JSON.stringify(emo.emo_vec || [0, 0, 0, 0, 0, 0, 0, 0]),
                );
                $row.find(".siren-idx-vec-random-hidden").val(
                    emo.emo_random ? "true" : "false",
                );
                ...
                $row.find(
                    ".siren-idx-audio-mode-ui .siren-idx-audio-input",
                ).val(emo.ref_audio || "");
                if (emo.emo_weight !== undefined && emo.emo_weight !== null) {
                    $row.find(
                        ".siren-idx-audio-mode-ui .siren-idx-emo-weight-input",
                    ).val(emo.emo_weight);
                }
            }
            $emoList.append($row);
...
export function bindIndexTtsEvents() {
    ...
    $("#siren-tts-provider-settings")
        ...
        .on("click", ".siren-idx-search-item", function () {
            ...
            const $container = $item
                .closest(".siren-idx-search-results")
                .parent();
    ...
    function updateTestDropdowns() {
        ...
        $("#siren-idx-emo-list .siren-ext-setting-row").each(function () {
            ...
            if (method === "audio") {
                ...
                randomVal =
                    $(this).find(".siren-idx-vec-random-hidden").val() ===
                    "true";
    ...
    $("#siren-idx-test-generate")
        ...
        .on("click", async function () {
            ...
            if (!charAudioPath)
                return window.toastr?.warning("请先选择角色音色！");
            ...
            try {
                ...
                const payload = {
                    ...
                    temperature:
                        parseFloat($("#siren-idx-param-temp").val()) || 0.8,
                    ...
                    length_penalty:
                        parseFloat($("#siren-idx-param-len").val()) || 0.0,
                    max_mel_tokens:
                        parseInt($("#siren-idx-param-mel").val()) || 1500,
                };
                ...
                if (moodVal === "3") {
                    ...
                    payload.emo_text =
                        $("#siren-idx-test-emo-text").val().trim() || text;
                ...
                $btn.html('<i class="fa-solid fa-bolt"></i> 生成').prop(
                    "disabled",
                    false,
                );
            }
    ...
    $("#siren-indextts-check")
        ...
        .on("click", async function (e) {
            ...
            try {
                ...
                if (res.ok) {
                    if (window.toastr)
                        window.toastr.success("IndexTTS 引擎连接成功！");
                    // 变成绿色打勾
                    $btn.html(
    ...
    $("#siren-idx-emo-list")
        ...
        .on("click", ".siren-idx-vec-open-btn", function () {
            ...
            const savedWeight = $(this)
                ...
                .val();
            const isRandom =
                $(this).siblings(".siren-idx-vec-random-hidden").val() ===
                "true";
    ...
    $("#siren-idx-vec-save").on("click", function () {
        ...
        currentRowVectorInput
            ...
            .val(
                $("#siren-idx-modal-random-cb").is(":checked")
                    ? "true"
                    : "false",
            );
    ...
    const performUnifiedSave = async (isGlobalButton = false) => {
        ...
        $("#siren-idx-char-list .siren-ext-setting-row").each(function () {
            const charName = $(this)
                ...
                .val()
                .trim();
            const voicePath = $(this)
                .find(".siren-idx-audio-input")
                .val()
                .trim();
        ...
        $("#siren-idx-emo-list .siren-ext-setting-row").each(function () {
            ...
            if (method === "audio") {
                ...
                const weightVal = $(this)
                    ...
                    .val();
                preset.emo_weight =
                    weightVal === "" ? null : parseFloat(weightVal);
                preset.emo_vec = null;
                ...
                preset.emo_random =
                    ...
                    "true";
                const weightVal = $(this)
                    .find(".siren-idx-vec-weight-hidden")
                    .val();
                preset.emo_weight =
                    weightVal === "" ? null : parseFloat(weightVal);
