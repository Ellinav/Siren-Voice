                ...
                worldbook.forEach((entry) => {
                    if (entry.name && entry.name.startsWith("TTS-")) {
                        ...
                        if (entry.enabled) {
                            ...
                            if (
                                typeof entry.extra.siren_original_content !==
                                "string"
                            ) {
                                entry.extra.siren_original_content =
                                    entry.content;
                            }
...
export async function syncSpatialWorldbookEntries(spatialMode) {
    if (
        ...
        console.warn(
            "[Siren Voice] TavernHelper 不可用，跳过空间感世界书同步。",
        );
        return;
    }
    ...
    try {
        ...
        console.log(
            `[Siren Voice] 空间感世界书同步完成。当前模式: ${spatialMode}`,
        );
    } catch (error) {
...
export async function syncBgmWorldbookEntries(isBgmEnabled) {
    ...
    // 🌟 新增：获取当前的 BGM 和 SFX 列表
    const settings = getSirenSettings();
    const bgmState = settings.bgm || {};

    // 提取 BGM 名称数组并过滤掉空名称
    const bgmLib = bgmState.libraries?.[bgmState.current_list] || [];
    const bgmNames = bgmLib
        .map((item) => item.name)
        .filter((name) => name.trim() !== "");
    const bgmStr = bgmNames.length > 0 ? `[${bgmNames.join(", ")}]` : "[]";

    // 提取 SFX 名称数组并过滤掉空名称
    const sfxLib = bgmState.sfx_libraries?.[bgmState.sfx_current_list] || [];
    const sfxNames = sfxLib
        .map((item) => item.name)
        .filter((name) => name.trim() !== "");
    const sfxStr = sfxNames.length > 0 ? `[${sfxNames.join(", ")}]` : "[]";
...
export async function syncMusicWorldbookEntry(isMusicEnabled) {
    if (
        ...
        console.warn(
            "[Siren Voice] TavernHelper 不可用，跳过 Music 世界书同步。",
        );
        return;
    }
