...
async function playSingleBlob(blob, speakObj = null) {
    ...
    try {
        ...
        console.warn(
            "[Siren Voice] TTS 空间/特效路由失败，尝试回退原生控制",
            e,
        );
...
export async function fetchTtsBlobProvider(
    ...
    try {
        ...
        // 1. 查找缓存时，必须使用带有语气词的原始 speakObj.text，保证 Cache Hit
        if (!forceRegen && currentChatId) {
            const cachedRecord = await findExactTtsRecord(
                ...
                speakObj.char,
                speakObj.text,
                speakObj.mood || "", // 👈 新增：传入 mood 参与查库校验
                speakObj.detail || "", // 👈 新增：传入 detail 参与查库校验
            );
        ...
        switch (provider) {
            ...
            case "minimax":
                ...
                // 💡 新增：将方括号替换为 MiniMax API 需要的小括号
                const preloadMmText = speakObj.text
                    .replace(/\[([^\]]+)\]/g, "($1)")
                    .replace(/【([^】]+)】/g, "($1)");

                // 传入替换好的 preloadMmText
                ...
                break;
            case "doubao":
                const dbConfig = getDoubaoCharConfig(
                    speakObj.char,
                    ttsSettings,
                );
            ...
            default:
                console.warn(
                    `[Siren Voice][预加载] 暂不支持该引擎: ${provider}`,
                );
                return null;
        }
        ...
        if (blob && currentChatId) {
            addTtsRecord({
                ...
                char: speakObj.char,
                text: speakObj.text,
                mood: speakObj.mood || "", // 👈 新增：将 mood 存入数据库
                detail: speakObj.detail || "", // 👈 新增：将 detail 存入数据库
                floor,
