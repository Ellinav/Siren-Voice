 ...
 * 智能点歌的单曲搜索 (复用列表搜索)
 */
export async function searchSong(keyword, source = "netease") {
    const results = await searchSongList(keyword, source, 5);
    ...
    for (let line of lines) {
        ...
        if (match) {
            const timeInSeconds =
                parseInt(match[1]) * 60 +
                parseInt(match[2]) +
                parseInt(match[3].length === 2 ? match[3] * 10 : match[3]) /
                    1000;
 ...
 * 智能配乐入口：只给名字去盲搜
 */
export async function playTargetSong(title, artist, source = "netease") {
    const keyword = artist ? `${title} ${artist}` : title;
    updatePlayerUI(title, artist || "搜索中...", null, true);
    updateLyricUI("正在打捞潜流...");

    let trackInfo = await searchSong(keyword, source);
    if (!trackInfo && artist) trackInfo = await searchSong(title, source);
    ...
    // 搜到了，转交给精准播放核心
    await playExactSong(trackInfo);
}

/**
 * 🌟 新增：精准播放核心 (传入带有 ID 的对象，拒绝重复盲搜)
 */
export async function playExactSong(trackInfo) {
...
function executePlay(url, title, artist, picUrl) {
    ...
    const renderLoop = (sysTime) => {
        ...
        if (deltaMs > 100)
            fpsStatus = `❌ 严重卡死 (间隔:${deltaMs.toFixed(1)}ms)`;
    ...
    currentAudio
        .play()
        .catch((e) => console.error("[Siren Voice] 播放失败:", e));
 ...
 */
export function setPlaylistContext(playlist, targetSongObj) {
    currentPlaylist = playlist;
    currentSongIndex = playlist.findIndex((s) => {
        // 如果是从深海歌单来的，比对 ID
        if (s.id && targetSongObj.id) return s.id === targetSongObj.id;
        // 如果是从潜流回响来的，比对歌名和歌手
        return (
            s.name === targetSongObj.name && s.artist === targetSongObj.artist
        );
    });
    // 防止找不到时变 -1 导致报错
...
export async function playNext() {
    ...
    if (nextSong.id && !String(nextSong.id).startsWith("echo_")) {
        await playExactSong(nextSong); // 有 ID，精准拉取
    } else {
        const defaultSource = getSirenSettings().music.source || "netease";
        await playTargetSong(
            nextSong.name,
            nextSong.artist,
            nextSong.source || defaultSource,
        ); // 无 ID，重新盲搜
    }
}
...
export async function playPrev() {
    ...
    if (prevSong.id && !String(prevSong.id).startsWith("echo_")) {
        await playExactSong(prevSong);
    } else {
        const defaultSource = getSirenSettings().music.source || "netease";
        await playTargetSong(
            prevSong.name,
            prevSong.artist,
            prevSong.source || defaultSource,
        );
    }
}
