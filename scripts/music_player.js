        ...
        <div id="siren-music-player" class="siren-ext-player-pill" style="display: none; top: 55px; left: calc(100vw - 320px); transform: translate(${savedPos.x}px, ${savedPos.y}px);">
            <div class="siren-ext-player-basic">
                ...
                <div class="siren-ext-player-info">
                    <div class="siren-ext-song-title">等待共鸣</div>
...
function bindPlayerEvents() {
    ...
    function drag(e) {
        if (isDragging) {
            ...
            if (baseRect) {
                ...
                const maxY =
                    window.innerHeight - baseRect.top - baseRect.height;
    ...
    if (modeBtnPill) {
        ...
        modeBtnPill.addEventListener("click", function () {
            const currentMode =
                getSirenSettings().music.play_mode || "sequential";
            ...
            const panelIcon = document.getElementById(
                "siren-music-play-mode-icon",
            );
            if (panelIcon) {
...
export function updatePlayerUI(title, artist, picUrl, isLoading = false) {
    ...
    const titleEl = player.querySelector(".siren-ext-song-title");
    const artistEl = player.querySelector(".siren-ext-song-artist");
...
export function updateLyricToggleUI(hasTranslation, currentMode) {
    ...
    if (!hasTranslation) {
        ...
        toggleBtn.title =
            currentMode === "tlyric" ? "切换至原文" : "切换至译文";
    }
}
...
export function setPlayerWarningState(isWarning) {
    ...
    // 🌟 同样限定在真药丸内部查找封面图标
    const coverIcon = player.querySelector(
