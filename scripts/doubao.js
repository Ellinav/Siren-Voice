...
export function bindDoubaoEvents() {
    ...
    $("#siren-db-test-generate")
        ...
        .on("click", async function () {
            ...
            if (!text)
                return (
                    window.toastr && window.toastr.warning("测试文本不能为空")
                );
            ...
            if (!config)
                return (
                    window.toastr &&
                    window.toastr.error("未能读取到该角色的音色配置")
                );
            ...
            try {
                ...
                $("#siren-db-test-status")
                    ...
                    .css("color", "#ef4444");
                if (window.toastr)
                    window.toastr.error(err.message || "请求失败");
            } finally {
                // 解除按钮锁定
...
function addCharRow(name, model, speaker) {
    ...
    const rowHtml = `
        <div id="${rowId}" class="siren-ext-setting-row siren-db-char-row" style="display: flex; gap: 8px; padding: 12px; background: rgba(0,0,0,0.25); border: 1px solid rgba(255,255,255,0.05); border-radius: 8px; margin-bottom: 4px; align-items: center;">
            
            <input type="text" class="siren-ext-input siren-db-char-name" placeholder="角色名" value="${name}" style="flex: 1.2; min-width: 80px; height: 32px; box-sizing: border-box; margin: 0;">
            
            <select class="siren-ext-select siren-db-model-select" style="flex: 1.5; min-width: 120px; height: 32px; box-sizing: border-box; margin: 0;">
            ...
            <div class="siren-db-speaker-container" style="flex: 2; min-width: 130px; display: flex; align-items: center;">
                </div>
            
            <button class="siren-ext-btn siren-ext-btn-secondary siren-db-btn-del" style="height: 32px; padding: 0 10px; box-sizing: border-box; margin: 0; color: #ef4444; flex-shrink: 0;" title="删除">
    ...
    function renderSpeakerField(currentModel, currentValue) {
        if (currentModel === "seed-tts-2.0") {
            ...
            `);
            if (currentValue)
                $speakerContainer.find("select").val(currentValue);
        } else {
            $speakerContainer.html(`
